...
pub fn go_named_link(ctx: &mut ActionContext, link_name: &[u8]) -> Result<()> {
    ...
    let Some(link_id) = ctx.world.links.iter().position(|x| x.name == link_name) else {
        ...
        ctx.printer.nlc(2);
        return Err(ActionError::NoSuchExit);
    };
    ...
    let (from_location_id, to_location_id) = {
        ...
        let to_location_id =
            if let Some(lnk) = from_location.links.iter().find(|x| x.link == link_id) {
                ...
                ctx.printer.p(b"cannot go there");
                ctx.printer.nlc(2);
                return Err(ActionError::CannotGoThere);
            };
...
pub fn take(ctx: &mut ActionContext) -> Result<()> {
    ...
    let Some(object_name) = ctx.tokens.next() else {
        ...
        ctx.printer.nlc(2);
        return Err(ActionError::TakeWhat);
    };

    {
        ...
        let Some((object_index, &object_id)) = location
            ...
            ctx.printer.nlc(2);
            return Err(ActionError::ObjectNotHere);
        };
...
pub fn drop(ctx: &mut ActionContext) -> Result<()> {
    let Some(object_name) = ctx.tokens.next() else {
        ...
        ctx.printer.nlc(2);
        return Err(ActionError::DropWhat);
    };
...
pub fn give(ctx: &mut ActionContext) -> Result<()> {
    ...
    let Some(to_entity_name) = ctx.tokens.next() else {
        ...
        ctx.printer.nlc(2);
        return Err(ActionError::GiveToWhom);
    };
    ...
    let Some(object_name) = ctx.tokens.next() else {
        ...
        ctx.printer.nlc(2);
        return Err(ActionError::GiveWhat);
    };
    ...
    let Some(&to_entity_id) = ctx.world.locations[ctx.world.entities[ctx.entity].location]
        ...
        ctx.printer.p(b" not here");
        ctx.printer.nlc(2);
        return Err(ActionError::EntityNotHere);
    };
...
pub fn new_location(ctx: &mut ActionContext) -> Result<()> {
        ...
        .any(|x| x.link == to_link_id)
    {
        ...
        ctx.printer.nlc(2);
        return Err(ActionError::LinkFromLocationAlreadyExists);
    }
...
pub fn new_entity(ctx: &mut ActionContext) -> Result<()> {
    ...
    let Some(entity_name) = ctx.tokens.next() else {
        ...
        ctx.printer.nlc(2);
        return Err(ActionError::WhatEntityName);
    };
...
pub fn tell(ctx: &mut ActionContext) -> Result<()> {
    let Some(to_name) = ctx.tokens.next() else {
        ...
        ctx.printer.nlc(2);
        return Err(ActionError::TellToWhom);
    };
